...
public class CustomerController {
	...
	private List<Items> getAllItems() {
	 ...
	 private List<Items> availableItems = Arrays.asList(
		        new Items(1, "Chicken Biryani", 140.0),
		        new Items(2, "Muttom Biryani", 200.0),
		        new Items(3, "Kara Pongal", 80.0),
...
//		model.addAttribute("items", listItems);
	...
	public String addOrder(@RequestParam("itemIds") List<Integer> itemIds,Order order1, Model model) {
		...
		Order order = new Order(1, selectedItems); // You can generate the order ID dynamically
		model.addAttribute("order", order); // Add the order to the model
